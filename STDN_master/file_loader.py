...
class file_loader:
    ...
    hist_feature_daynum = 7, last_feature_num = 48, nbhd_size = 1, cnn_nbhd_size = 3):
        ...
        folder_name = './sampler_data/' + datatype + '_' + str(att_lstm_num) + '_' + str(long_term_lstm_seq_len) + '_' + \
             str(short_term_lstm_seq_len) + '_' + str(hist_feature_daynum) + '_' + str(last_feature_num) + '_' + str(nbhd_size) + '_' + str(cnn_nbhd_size)

        # if Path(folder_name).is_dir():

        #     output_cnn_att_features = np.load(folder_name + '/output_cnn_att_features.npy').tolist()
        #     output_flow_att_features = np.load(folder_name + '/output_flow_att_features.npy').tolist()
        #     lstm_att_features = np.load(folder_name + '/lstm_att_features.npy')
        #     cnn_features = np.load(folder_name + '/cnn_features.npy')
        #     flow_features = np.load(folder_name + '/flow_features.npy')
        #     short_term_lstm_features = np.load(folder_name + '/short_term_lstm_features.npy')
        ...
        time_start = (hist_feature_daynum + att_lstm_num + offset) * self.timeslot_daynum + long_term_lstm_seq_len
